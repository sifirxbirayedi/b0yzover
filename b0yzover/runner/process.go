...
func Process(config *Config) error {
	...
	collectorWg.Add(1)
	go collectResults(resCh, &results, config, &collectorWg)
...
func collectResults(resCh <-chan *subdomainResult, results *[]*subdomainResult, config *Config, wg *sync.WaitGroup) {
	defer wg.Done()
	for r := range resCh {
...
func RunDiscoveryScan(domain string, scan *Config) error {
	...
	subdomains, err := FetchSubdomains(domain)
	if err != nil {
		return err
	}
	if len(subdomains) == 0 {
